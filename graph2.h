...
public:
  ...
  private:
    ...
    class Neighborout {
  public:
        Neighborout(Graph::NodeId n, double w,int nummer);
        double edge_weight() const;
        Graph::NodeId id() const;
        int knummer() const;
  private:
  ...
  public:
        ...
        void add_neighborout(Graph::NodeId nodeid, double weight, int nummer);
        const std::vector<Neighbor> & adjacent_nodes() const;
        const std::vector<Neighbor> & adjacent_nodesout() const;
  private:
        std::vector<Neighbor> _neighbors;
        std::vector<Neighbor> _neighborsout;
    };
  ...
  void add_edgeout(NodeId tail, NodeId head,int nummer, double weight = 1.0);
