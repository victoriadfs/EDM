...
Graph::Neighborout::Neighborout(Graph::NodeId n, double w, int nummer): _id(n), _edge_weight(w),_knummer(nummer){
}
...
void Graph::add_edgeout(NodeId tail, NodeId head, int nummer, double weight)
{
    if (tail >= num_nodes() or tail < 0 or head >= num_nodes() or head < 0) {
        throw std::runtime_error("Edge cannot be added due to undefined endpoint.");
    }
    _nodes[tail].add_neighborout(head, weight, nummer);
    if (dirtype == Graph::undirected) {
        _nodes[head].add_neighborout(tail, weight, nummer);
    }
}
...
void Graph::Node::add_neighborout(Graph::NodeId nodeid, double weight, int nummer)
{
    _neighborsout.push_back(Graph::Neighbor(nodeid, weight, nummer));
}
...
const std::vector<Graph::Neighbor> & Graph::Node::adjacent_nodesout() const
{
    return _neighborsout;
}
...
Graph::NodeId Graph::Neighborout::id() const
{
    return _id;
}

double Graph::Neighborout::edge_weight() const
{
    return _edge_weight;
}


int Graph::Neighborout::knummer() const
{
    return _knummer;
}
...
void Graph::print() const
{
    ...
    for (auto nodeid = 0; nodeid < num_nodes(); ++nodeid) {
        ...
        for (auto neighbor: _nodes[nodeid].adjacent_nodesout()) {
...
Graph::Graph(char const * filename, DirType dtype): dirtype(dtype)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (tail != head) {
            ...
            add_edgeout(tail, head, nummer, weight);
        }
        else {
